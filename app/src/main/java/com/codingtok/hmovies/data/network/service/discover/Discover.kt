...
class Discover {
    ...
    class MovieBuilder : HashMap<String, String>() {
        ...
        fun watchRegion(region: String?) = apply { clearPut("watch_region", region) }
        fun withTextQuery(query: String?) = apply { clearPut("with_text_query", query) }
        fun withWatchMonetizationTypes(query: WatchMonetizationTypes = WatchMonetizationTypes.FLATRATE) = apply { clearPut("with_watch_monetization_type", query.toString()) }
    ...
    enum class WatchMonetizationTypes(private val queryValue: String) {
        FLATRATE("flatrate"),
        FREE("free"),
        ADS("ads"),
        RENT("rent"),
